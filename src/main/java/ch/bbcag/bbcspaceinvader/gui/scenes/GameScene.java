...
public class GameScene implements KeyControllable {
    ...
    private void update(double deltaInSec) {
        ...
        if (isSpaceKeyPressed && isShipBatteryFull(deltaInSec) && spaceship.AMMO > 0) {
            ...
            if ((spaceship.AMMO <= 10 && minigun) == true) {
                displayNotification("Minigun disabled");
                minigun = false;
            }
        ...
        if (kills >= 50){
            displayNotification("Achievement Unlocked: 50+ Kills reached");
        }
    }
    ...
    private void showInputDialog() {
        Platform.runLater(() -> {
            ...
            dialog.showAndWait().ifPresent(result -> {
                ...
                if ("wave+14".equalsIgnoreCase(result.trim())) {
                    wave = 14;
                    displayNotification("Set wave to 14!");
                }
    ...
    private void switchToVictoryScene() {
        VictoryScene victoryScene = new VictoryScene(navigator, this);
        navigator.navigateTo(victoryScene.getScene());
        resetGame();
    }
